...
# AudioProcessor (consolidated for single file deployment)
class AudioProcessor:
    ...
    def analyze_audio_video_duration(audio_path: str, video_duration: float) -> dict:
        ...
        try:
            ...
            fade_out_duration = min(3.0, video_duration * 0.1)
...
# Chunked processor for memory management
class ChunkedProcessor:
    @staticmethod
    def process_with_memory_limit(operation, **kwargs):
        return operation()
        
    @staticmethod
    def process_files_in_chunks(file_paths, processor_func, chunk_size=3, memory_manager=None):
        results = []
...
# Duplicate VideoUploadHandler removed - using the one defined above

# Duplicate AudioProcessor removed - using the simplified one defined abovet
        """
        try:
